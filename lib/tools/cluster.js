...
exports.initConnection = function (targetConfig, socket, usePath) {
  ...
  connection.baseUrl  = tools.net.getUri(targetConfig);
  connection.uri      = connection.baseUrl;
  connection.parent   = socket;
  connection.queue    = queue.new();
  connection.path     = tools.uri.getPathString(
    targetConfig.port,
...
exports.connect = function connect (targetSocket) {
  ...
  var connections = exports.getConnections(targetSocket, socket, socket.config.usePath);

  this.lib._connect(connections);
};
